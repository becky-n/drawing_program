 ...
 * @param {number} x button centre x
 * @param {number} y button centre y
...
class InteractiveButton extends InteractiveObject {
    ...
    update(){
        ...
        let fill = this.fill;
        // changes colour to show button is selected
        if(InteractiveButton.selected ===this){
            fill = this.selected
        // changes colour when mouse hovers over button
        }else if (this.inBounds){
            fill =this.over
        }
        // calls draw button function
...
class Swatch extends InteractiveObject {
    ...
    update(){
       ...
       // changes stroke colour to show button is selected
        if(Swatch.selected ===this){
           stroke = this.strokeSel;
        }
        // changes stroke colour when mouse hovers over button
        else if (this.inBounds){
            stroke =this.overSel
        }
        // calls draw button function
...
class Size extends InteractiveButton {
    // includes all of interactive button code
    update(){
        // checks if mouse is on button
        this.inBounds = this.getBoundary(this.x,this.y, this.w, this.h,
            this.xMouse, this.yMouse)
        ...
        if(Size.selected ===this){
            // changes colour of button if selected
            fill = this.selected
        }else if (this.inBounds){
            // changes colour of button if mouse hovers over
            fill =this.over
        }
        this.draw(this.x,this.y,this.w,this.h,fill, this.stroke,this.text,
            // calls draw button function
